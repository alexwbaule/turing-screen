...
class ThemeCanvas(Gtk.ScrolledWindow):
    ...
    def _load_measurement(self, m, base: str):
        ...
        if m.PercentText:
            self._add_text_element(m.PercentText, base + ".PERCENT_TEXT")
        if m.Graph:
            self._add_graph_element(m.Graph, base + ".GRAPH")
        if m.Radial:
    ...
    def _add_text_element(self, data: Text, path: str):
    ...
    def _add_graph_element(self, data: Graph, path: str):
        if data.SHOW is False:
            return
        try:
    ...
    def _add_radial_element(self, data: Radial, path: str):
        if data.SHOW is False:
            return
        try:
    ...
    def _add_chart_element(self, data: Chart, path: str):
        if data.SHOW is False:
            return
        try:
